...
class MLP(torch.nn.Module):
    ...
    def __init__(self, d_in, d_hidden, d_out, dropout=0.5, num_layers=2):
        super(MLP, self).__init__()

        layers = []
        layers.append(nn.Linear(d_in, d_hidden))
        layers.append(nn.BatchNorm1d(num_features=d_hidden))
...
def train(
    ...
    if pred_type == 'classification':
        ...
        Y_train = torch.Tensor(train_dataset.Y).to('cpu')
    ...
    with tqdm(range(epochs), unit='epochs', disable=tune) as tepoch: 
        for epoch in tepoch:
            ...
            if tune:
                os.makedirs(f'mlp_{epoch}', exist_ok=True)
                torch.save((model.state_dict(), optimizer.state_dict()), f'mlp_{epoch}/checkpoint.pt')
                checkpoint = air.checkpoint.Checkpoint.from_directory(f'mlp_{epoch}')
...
def eval(model, dataloader, criterion, prefix=''):
    '''Evaluation function for MLP, compatible with Ray Tune.'''
    ...
    # TODO: Log the plots via W&B
    #if output_dir is not None and pred is not None:
    #    direction = ('eicu', 'mimic') if eicu else ('mimic', 'eicu')
    #    plot_PR_curve(labels, probs, auprc, 'MLP', pred, output_dir, *direction)
    #    plot_ROC_curve(labels, probs, auroc, 'MLP', pred, output_dir, *direction)
